...
const blacklist_data = fs.readFileSync("./src/blacklist.json");
const blacklist = JSON.parse(blacklist_data);
console.log("Loaded " + blacklist.usdt_blacklist.length + " banned addresses for USDT");
...
async function handleTransaction(txEvent) {
  ...
  for (let i = 0; i < blacklist.usdt_blacklist.length; i++) {
    let address = blacklist.usdt_blacklist[i];
